...
class TorchSmokeBackend(StageBackend):
    ...
    def run_stage(
        ...
        if self.require_cuda and not cuda_available:
            raise RuntimeError("torch-smoke backend requires CUDA, but torch.cuda is unavailable")
        ...
        payload = {
            ...
            "stage_smoke": {
                "seed": seed,
                "metric": metric,
                "note": "This is an executable GPU/torch smoke artifact, not a real model checkpoint.",
            },
        }
        if stage.type == "data_ingestion":
            payload["lineage_report"] = build_data_lineage_report(
...
def create_backend(name: str, require_cuda: bool = False) -> StageBackend:
    if name == "manifest":
...
def _run_torch_stage_smoke(torch: Any, device: Any, seed: int, stage: StageConfig) -> dict[str, Any]:
